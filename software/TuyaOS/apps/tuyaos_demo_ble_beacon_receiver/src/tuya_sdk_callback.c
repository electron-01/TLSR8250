...
STATIC VOID_T tuya_ble_evt_callback(TAL_BLE_EVT_PARAMS_T *p_event)
{
    switch (p_event->type) {
        ...
        case TAL_BLE_EVT_ADV_REPORT: {
            if ((p_event->ble_event.adv_report.p_data[13] == 0x80) && \
                (p_event->ble_event.adv_report.p_data[11] == 0x40) && \
                (p_event->ble_event.adv_report.p_data[12] == 0x80) && \
                (p_event->ble_event.adv_report.p_data[30] == 0x00)) {
                UINT8_T buf[41];
                UINT8_T index = 0;
                memcpy(buf, &p_event->ble_event.adv_report.peer_addr.type, SIZEOF(TAL_BLE_ADDR_T));
                index += SIZEOF(TAL_BLE_ADDR_T);
                buf[index++] = p_event->ble_event.adv_report.adv_type;
                buf[index++] = p_event->ble_event.adv_report.rssi;
                memcpy(buf + index, p_event->ble_event.adv_report.p_data, p_event->ble_event.adv_report.data_len);
                index += p_event->ble_event.adv_report.data_len;
                buf[index++] = p_event->ble_event.adv_report.data_len;
                tuya_ble_custom_evt_send_with_data(APP_EVT_0, buf, index);
            }
