    ...
    switch (event->evt) {
        case TUYA_BLE_CB_EVT_CONNECT_STATUS: {
            if (event->connect_status == BONDING_CONN) {
                ...
                app_pairing_stop();
                if (app_status_info.bound_flag == 0) {
                    app_status_info.bound_flag = 1;
#if ENABLE_LED
                    app_led_start(APP_LED_PAIRING_SUCCESS);
#endif
                } else {
#if ENABLE_LED
                    app_led_stop(APP_LED_PAIRING_START);
#endif
                }
        ...
        case TUYA_BLE_CB_EVT_TIME_STAMP: {
            ...
            app_status_info.unix_time_status = 1;
