...
STATIC VOID_T tuya_ble_evt_callback(TAL_BLE_EVT_PARAMS_T *p_event)
{
    switch (p_event->type) {
        ...
        case TAL_BLE_EVT_PERIPHERAL_CONNECT: {
            ...
            tal_oled_show_string(120, 0, (VOID_T*)"*", 16);
...
VOID_T tal_ble_beacon_cb(VOID_T *arg)
{
    switch (g_dp_report_count) {
        case 0: {
            g_cmd.dp_id = 101;
            g_cmd.dp_data_len = 1;
            memcpy(g_cmd.dp_data, &g_dp_value.led, g_cmd.dp_data_len);
        ...
        case 1: {
            g_cmd.dp_id = 102;
            g_cmd.dp_data_len = 1;
            memcpy(g_cmd.dp_data, &g_dp_value.charge_state, g_cmd.dp_data_len);
            app_dp_report(g_cmd.dp_id, g_cmd.dp_data, g_cmd.dp_data_len);
        ...
        case 2: {
            ...
            UINT32_T tmp_temperature = g_dp_value.temperature;
            tal_util_reverse_byte(&tmp_temperature, SIZEOF(UINT32_T));

            memcpy(g_cmd.dp_data, &tmp_temperature, g_cmd.dp_data_len);
            app_dp_report(g_cmd.dp_id, g_cmd.dp_data, g_cmd.dp_data_len);
        ...
        case 3: {
            g_cmd.dp_id = 104;
            g_cmd.dp_data_len = 9;
            memcpy(g_cmd.dp_data, &g_dp_value.welcome, g_cmd.dp_data_len);
            app_dp_report(g_cmd.dp_id, g_cmd.dp_data, g_cmd.dp_data_len);
        ...
        case 4: {
            g_cmd.dp_id = 105;
            g_cmd.dp_data_len = 5;
            memcpy(g_cmd.dp_data, &g_dp_value.custom_data, g_cmd.dp_data_len);
            app_dp_report(g_cmd.dp_id, g_cmd.dp_data, g_cmd.dp_data_len);
        ...
        case 5: {
            ...
            UINT32_T tmp_fault_alarm = g_dp_value.fault_alarm;
            tal_util_reverse_byte(&tmp_fault_alarm, SIZEOF(UINT32_T));

            memcpy(g_cmd.dp_data, &tmp_fault_alarm, g_cmd.dp_data_len);
            app_dp_report(g_cmd.dp_id, g_cmd.dp_data, g_cmd.dp_data_len);
...
OPERATE_RET tuya_init_last(VOID_T)
{
    ...
    if (tal_oled_init() == OPRT_OK) {
        ...
        uint8_t addr_str[13] = {0};
        tal_ble_address_get(&addr);
        tal_util_reverse_byte(addr.addr, 6);
        tal_util_str_hexarray2hexstr(true, addr.addr, 6, addr_str);

        tal_oled_show_string(0,  0, (VOID_T*)addr_str, 12);

        tal_oled_show_string(0, 2, (VOID_T*)"Adv14: ", 16);
