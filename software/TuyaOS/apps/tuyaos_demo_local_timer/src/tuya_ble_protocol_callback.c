...
STATIC VOID_T tuya_ble_protocol_callback(tuya_ble_cb_evt_param_t* event)
{
    switch (event->evt) {
        ...
        case TUYA_BLE_CB_EVT_UNBOUND: {
            ...
            tal_local_timer_info_delete_all();
        } break;
...
//            TAL_PR_HEXDUMP_INFO("TUYA_BLE_CB_EVT_DP_QUERY", event->dp_query_data.p_data, event->dp_query_data.data_len);
        ...
        case TUYA_BLE_CB_EVT_APP_PASSTHROUGH_DATA: {
            TAL_PR_INFO("TUYA_BLE_CB_EVT_APP_PASSTHROUGH_DATA");
            if (event->app_passthrough_data.type == TAL_BLE_APP_PASSTHROUGH_SUBCMD_LOCAL_TIMER) {
                tal_local_timer_handler(&event->app_passthrough_data);
            }
        } break;
